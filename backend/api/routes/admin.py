...
async def get_admin_stats(request: Request, current_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        logger.info(
            f"📊 Stats: {total_users} users, {total_conversations} conversations, {total_documents} documents, {total_messages} messages"
        )
...
async def get_system_health(current_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        logger.info(
            f"🏥 Health check: Supabase {db_time}ms, Railway ✓, Anthropic {health_data['anthropic']['status']}, Voyage {health_data['voyageAI']['status']}, Neo4j {health_data['neo4j']['status']} ({neo4j_time}ms)"
        )
...
async def get_interface_health(current_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        logger.info(
            f"📊 Interface health: {avg_response_length} avg words, {len(stuck_conversations)} stuck, {total_conversations_7d} chats, {active_users_count} active users"
        )
...
async def get_help_document(document_id: str, current_user: dict = Depends(require_admin)):
...
async def get_help_analytics(
    ...
    try:
        ...
        messages_result = await asyncio.to_thread(
            ...
            .select(
                "id, conversation_id, role, content, sources, feedback, feedback_timestamp, timestamp"
            )
        ...
        logger.info(
            f"📊 Help analytics: {len(user_questions)} questions, {len(low_confidence_responses)} low confidence, avg confidence {avg_confidence}"
        )
...
async def get_keyword_trends(current_user: dict = Depends(require_admin), days: int = 30):
    ...
    try:
        ...
        logger.info(
            f"📊 Keyword trends: {len(keywords)} keywords, {len(questions)} questions, {domain_keyword_count} L&D terms"
        )
