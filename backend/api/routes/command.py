...
async def command_stream(request: Request, command_request: CommandRequest):
    """Stream command responses with tool_use loop."""
    logger.info(
        "Command request received",
        extra={"message_length": len(command_request.message)},
    )
    ...
    async def generate():
        try:
            ...
            for iteration in range(MAX_TOOL_ITERATIONS):
                ...
                # Execute tool calls
                # TODO: execute_tool still takes supabase+client_id -- needs service-level migration
                tool_results = []
                for tu in tool_uses:
                    result = await execute_tool(tu["name"], tu["input"], None, None)
