    ...
    try:
        ...
        auth_result = await asyncio.to_thread(
            lambda: supabase.auth.admin.create_user(
                {"email": user_data.email, "password": temp_password, "email_confirm": True}
            )
        )
        ...
        await asyncio.to_thread(lambda: supabase.table("users").upsert(profile_data).execute())
...
async def delete_avatar(request: Request, user_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        await asyncio.to_thread(
            lambda: supabase.table("users").update({"avatar_url": None}).eq("id", user_id).execute()
        )
...
async def resend_invitation(request: Request, user_id: str, current_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        await asyncio.to_thread(lambda: supabase.auth.reset_password_for_email(email))
...
async def get_user_document_counts(current_user: dict = Depends(get_current_user)):
