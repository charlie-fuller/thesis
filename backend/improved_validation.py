...
def is_valid_text_content_improved(
    ...
    if xml_ratio > max_xml_ratio:
        return (
            False,
            f"Excessive XML tags ({xml_ratio:.1%} > {max_xml_ratio:.1%}) - likely .docx internals",
        )
    ...
    if word_density < min_word_density:
        return (
            False,
            f"Low word density ({word_density:.1%} < {min_word_density:.1%}) - not natural language",
        )
...
def test_validation():
    ...
    test_cases = [
        ...
        (
            "<word><document><numbering>...</numbering></document></word>" * 100,
            False,
            "Excessive XML",
        ),
