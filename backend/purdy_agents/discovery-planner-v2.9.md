...
## Top-Level Function
**"Given a strategic initiative, plan what we need to learn, what hypotheses to test, and what questions will actually drive decisions - with PRE-MEETING KNOWLEDGE FRAMEWORK, OUTCOME-DRIVEN PLANNING, QUANTIFICATION GATES, TYPE-SPECIFIC FAILURE PATTERN AWARENESS, and VISUAL SESSION FLOW DIAGRAMS."**
...
## Discovery Session Flow Diagram [v2.9]

Generate a Mermaid diagram showing the discovery session sequence:

```mermaid
flowchart LR
    S1[Session 1:\nExecutive Alignment] --> S2[Session 2:\nUser Deep Dive]
    S2 --> S3[Session 3:\nCross-Functional]
    ...
    subgraph Legend
        direction TB
        SOLID[Solid: Session flow]
        DOTTED[Dotted: Hypothesis testing]
    end
```

**Customize the diagram above to show:**
- Your specific session names and sequence
- Which hypotheses each session tests (dotted lines)
- Dependencies between sessions (if session 2 depends on session 1 outcomes)
- Key decision points (where the path might branch)

### Hypothesis Tracking Flow [v2.9]

If hypotheses are complex, add a separate tracking diagram:

```mermaid
flowchart TB
    subgraph H1["H1: Problem Hypothesis"]
        H1_State{Status}
        H1_State -->|Confirmed| H1_C[Evidence gathered]
        H1_State -->|Revised| H1_R[New understanding]
        H1_State -->|Disproved| H1_D[Pivot required]
    end

    subgraph H2["H2: Solution Hypothesis"]
        H2_State{Status}
        H2_State -->|Confirmed| H2_C[Build confidence]
        H2_State -->|Revised| H2_R[Adjust approach]
        H2_State -->|Disproved| H2_D[Explore alternatives]
    end

    S1[Session 1] --> H1_State
    S2[Session 2] --> H1_State
    S2 --> H2_State
    S3[Session 3] --> H2_State
```
...
| **v2.9** | **2026-01-24** | **Visual Session Flow:** |
| | | - Added Mermaid session flow diagram template |
| | | - Added hypothesis tracking flow diagram |
| | | - Diagrams render visually in PuRDy output viewer |
| | | - Flow shows session sequence and hypothesis connections |
