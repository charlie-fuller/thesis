...
def analyze_help_data(days: int = 90):
    ...
    messages_result = supabase.table('help_messages').select(
        'id, conversation_id, role, content, sources, feedback, timestamp'
    ...
    conversations_result = supabase.table('help_conversations').select(
        'id, user_id, title, created_at, help_type'
    ...
    for i, resp in enumerate(low_confidence_responses, 1):
        ...
        for source in resp['sources']:
            print(f"  - {source.get('title', 'Unknown')} / {source.get('section', 'Unknown')} ({source.get('similarity', 0):.1%})")
    ...
    topic_keywords = {
        ...
        'kpi': ['kpi', 'metric', 'analytics', 'dashboard', 'ideation', 'bradbury', 'correction', 'impact'],
    ...
    for topic in high_volume_topics:
        ...
        topic_low_conf = sum(1 for r in low_confidence_responses
                           if any(topic in str(s.get('title', '')).lower()
                                 for s in r['sources']))
        if topic_low_conf > 0:
