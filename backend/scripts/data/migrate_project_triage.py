...
def main():
    ...
    parser.add_argument('--source', type=str, help='Path to project-triage directory',
    ...
    parser.add_argument('--supabase-key', type=str, help='Supabase service key (or set SUPABASE_SERVICE_ROLE_KEY env var)')
    ...
    supabase_key = args.supabase_key or os.environ.get('SUPABASE_SERVICE_ROLE_KEY') or os.environ.get('SUPABASE_SERVICE_KEY')
    ...
    if profiles_dir.exists():
        for profile_file in profiles_dir.glob('*.md'):
            if profile_file.name in ('PROFILE-TEMPLATE.md', 'metrics-and-kpis.md', 'GNA-SYSTEMS-VIEW.md'):
                continue
