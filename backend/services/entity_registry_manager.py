...
class EntityRegistryManager:
    ...
    async def get_organization(
        self, client_id: str, org_id: Optional[str] = None, canonical_name: Optional[str] = None
    ...
    async def get_person(
        self, client_id: str, person_id: Optional[str] = None, canonical_name: Optional[str] = None
    ...
    async def learn_from_correction(
        ...
        if entity_type == "person":
            ...
            if entry:
                ...
                logger.info(f"Added '{original_value}' as alias for person '{corrected_value}'")
                return True
            else:
            ...
            entry = await self.get_organization(client_id, canonical_name=corrected_value)
            if entry:
                await self.add_organization_alias(entry.id, original_value)
                logger.info(f"Added '{original_value}' as alias for org '{corrected_value}'")
                return True
            else:
                await self.add_organization(
                    client_id=client_id, canonical_name=corrected_value, aliases=[original_value]
                )
                logger.info(f"Created org '{corrected_value}' with alias '{original_value}'")
                return True
    ...
    async def bootstrap_from_stakeholders(self, client_id: str) -> dict:
            ...
            for stakeholder in stakeholders:
                ...
                if name:
                    person_id = await self.add_person(
                        client_id=client_id, canonical_name=name, stakeholder_id=stakeholder_id
                    )
                    if person_id:
                ...
                if org and org not in orgs_seen:
                    orgs_seen.add(org)
                    org_id = await self.add_organization(client_id=client_id, canonical_name=org)
                    if org_id:
    ...
    async def get_correction_history(
        self, client_id: str, entity_type: Optional[str] = None, limit: int = 50
    ) -> list[dict]:
