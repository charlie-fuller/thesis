...
def create_research_task(topic: ResearchTopic, client_id: Optional[str] = None) -> str:
...
async def execute_research_task(task_id: str, topic: ResearchTopic) -> ResearchResult:
    ...
    try:
        ...
        # Store as document
        document_id = await store_research_document(topic=topic, content=content, task_id=task_id)
...
def get_research_scheduler_status() -> dict:
    ...
    return {"running": research_scheduler.running, "jobs": jobs}
