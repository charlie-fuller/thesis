...
async def extract_tasks_from_document(
    ...
    try:
        ...
        if not tasks:
            ...
            return {"status": "completed", "tasks_found": 0, "tasks_stored": 0}
...
async def accept_task_candidate(
    ...
    task_data = {
        ...
        if overrides
        else candidate["suggested_priority"],
        "due_date": overrides.get("due_date", candidate["suggested_due_date"])
        if overrides
        else candidate["suggested_due_date"],
        "assignee_user_id": user_id,
        "assignee_name": overrides.get("assignee_name", candidate.get("assignee_name"))
        if overrides
        else candidate.get("assignee_name"),
        "status": task_status,
...
async def bulk_action_candidates(
    ...
    return {"status": "completed", "action": action, "success": success, "failed": failed}
