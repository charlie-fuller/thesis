...
30. **Framing Extraction** - Three paths: (1) Inline "Generate from Documents" button in framing section triggers Discovery Guide and shows review panel without navigating away, (2) Run Agents tab for full agent control, (3) Discovery Agent in chat can discuss framing conversationally and propose structured `<framing_proposal>` blocks that users selectively apply via FramingProposalCard. Review panel appears whenever suggestions have content, regardless of existing framing state.
