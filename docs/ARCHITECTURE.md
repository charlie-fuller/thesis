...
45. **Iframe Theme Inheritance** - All embedded HTML visualizations in `frontend/public/` (maps, guides, process diagrams) sync with the parent app's theme. Script reads ThemeContext CSS custom properties and computes a full gray ramp via color interpolation. Supports dark/light detection with polling and MutationObserver for runtime changes.
...
47. **Manifesto Compliance System** - Background semantic evaluation of chat and meeting messages against manifesto principles. Claude Haiku scores messages for compliance. Three levels: aligned, drifting, misaligned. Compliance indicator dots on chat and meeting messages with expandable drill-down panels showing flagged messages, principle descriptions, why-flagged explanations, recommendations, and context-aware confidence ratings. Weekly digest scheduler. Admin analytics endpoint.
...
51. **Compliance Drill-Down** - Manifesto compliance indicators expand to show flagged message details: principle violations, why-flagged text enriched with principle descriptions, recommendations, and context-aware confidence ratings (high/medium/low) that adjust scores based on message context.
52. **In-App User Guides** - 6 interactive HTML guides embedded in the DISCO page plus 3 previously orphaned process maps surfaced via consolidated maps/guides tab with pill selector UI.
53. **Hub-Spoke Governance Redesign** - Color-coded hub-and-spoke governance visualization with flow paths, process owner/steward accountability framework, and consistent styling matching other platform maps.
54. **Project-Level DISCO** - DISCO agents operate at the project level with a streamlined four-agent workflow adapted for project-scoped discovery. Project detail modal includes DISCO agent runner panel.
55. **Discoveries-to-Initiatives Rename** - All user-facing labels renamed from "Discoveries" to "Initiatives" across the DISCO page, navigation, breadcrumbs, and component headers.
