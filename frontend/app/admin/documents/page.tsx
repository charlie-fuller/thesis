          ...
          <table className="w-full">
            <thead className="bg-page border-b border-default">
              <tr>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium uppercase text-muted">
                  Actions
                </th>
              </tr>
            </thead>
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <button
                      ...
                      className="link font-medium text-sm truncate max-w-xs block"
                      title={doc.filename}
                    >
                      {doc.filename}
                    </button>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className="p-1 text-muted hover:text-red-600 transition-colors cursor-pointer"
                        title="Delete"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="modal-title">
          <div className="bg-card rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="sticky top-0 bg-card border-b border-default px-6 py-4 flex items-center justify-between">
              <h2 id="modal-title" className="heading-3">Document Details</h2>
              <button
            ...
            <div className="px-6 py-4">
              {detailLoading ? (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-xs font-medium uppercase text-muted">Client</label>
                    <p className="text-sm text-primary mt-1">{documentDetails.client_name || 'Unknown'}</p>
                  </div>
                  ...
                  {documentDetails.uploaded_by_email && (
                    <div>
                      <label className="text-xs font-medium uppercase text-muted">Uploaded By</label>
                      <p className="text-sm text-primary mt-1">{documentDetails.uploaded_by_email}</p>
                    </div>
                  )}
                  {documentDetails.conversations && documentDetails.conversations.length > 0 && (
                    <div>
                      <label className="text-xs font-medium uppercase text-muted">Related Conversations</label>
                      <div className="mt-2 space-y-2">
                        {documentDetails.conversations.slice(0, 5).map((conv) => (
                          <div key={conv.id} className="text-sm text-secondary border-l-2 border-default pl-2">
                            {conv.title || 'Untitled'}
                          </div>
                        ))}
                      </div>
