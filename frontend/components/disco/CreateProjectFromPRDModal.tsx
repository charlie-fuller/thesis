...
interface CreateProjectFromPRDModalProps {
  open: boolean
  onClose: () => void
  initiativeId: string
...
function generateProjectCode(department: string): string {
  ...
  const prefix = prefixMap[department?.toLowerCase()] || 'PRD'
...
export default function CreateProjectFromPRDModal({
  open,
  onClose,
  initiativeId,
  initiativeName,
  prdId,
  prdTitle,
}: CreateProjectFromPRDModalProps) {
    ...
    const extractProject = async () => {
      ...
      try {
        const result = await apiPost<ExtractionResponse>(
          `/api/disco/initiatives/${initiativeId}/prds/${prdId}/extract-project`,
          {}
        )
        ...
        setError(err instanceof Error ? err.message : 'Failed to analyze PRD content')
      } finally {
        setExtracting(false)
      }
    }
  ...
  }, [projectCode, title, description, department, currentState, desiredState, roiPotential, implementationEffort, strategicAlignment, stakeholderReadiness, initiativeId, prdId, sourceContext, extractedTasks, selectedTaskIndices, onClose, router])
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Create Project from PRD</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {extracting ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-slate-600 dark:text-slate-400">Analyzing PRD content...</p>
              <p className="text-sm text-slate-500 dark:text-slate-500 mt-1">
                Extracting project details from &ldquo;{prdTitle}&rdquo;
              </p>
            </div>
              ...
              <div className="flex items-start gap-2 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg text-sm">
                ...
                <div>
                  <p className="text-indigo-900 dark:text-indigo-200">
                    Creating project from PRD: <strong>{prdTitle}</strong>
                  </p>
